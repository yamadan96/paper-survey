...
現在の **被災建物画像 多クラス損傷度分類（FIT2025 / IEICE2026、小規模な自前データセット・6 クラス）** との接続点:
...
2. **小規模データでは SimCLR ゼロから学習は成立しない → 継続事前学習ルート**
   - SimCLR は ImageNet 128 万枚前提。この規模で from scratch すると表現崩壊。
   - **推奨ルート**: ImageNet 事前学習済み ResNet/ViT → 自前データで SimCLR 継続事前学習 → ラベル付きデータで fine-tune。Reed et al. "Self-Supervised Pretraining Improves SSL Pretraining" (CVPR 2022) の枠組み。
   - **代替ルート**: xBD（約 24,000 枚の建物災害画像ペア、公開データ）で SimCLR 事前学習 → 自前データで fine-tune。論文として強い構成。
3. **Supervised Contrastive Learning がクラス不均衡対策として有力**
   - クラス不均衡がある。**少数クラスで標準 cross-entropy は学習困難**。
...
6. **実装着手の最短ルート（spike 1〜2 週間）**
   - Week 1: 自前データセットに対し 3 種比較を実装
     - Baseline A: ImageNet pretrain ResNet-18 → fine-tune
     - Baseline B: A の事前学習を **DINOv2 frozen + linear** に置換
     - Proposed: ImageNet pretrain → 自前データで SimCLR 継続事前学習（augmentation はドメイン特化版）→ fine-tune
...
- Supervised Contrastive Learning (Khosla et al., 2020) — クラス不均衡対策の本命。自前データに直接適用可能。
